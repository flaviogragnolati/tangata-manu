 ...
 */
export const ensureUserIsAuthed = t.middleware(({ ctx, next }) => {
...
export const ensureUserIsSuperadmin = t.middleware(({ ctx, next }) => {
  if (
    !ctx.session ||
    !ctx.session.user ||
...
export const ensureUserIsAdmin = t.middleware(({ ctx, next }) => {
  if (
    !ctx.session ||
    !ctx.session.user ||
    !['ADMIN', 'SUPERADMIN'].includes(ctx.session.user.role)
  ) {
    throw new TRPCError({ code: 'UNAUTHORIZED' });
  }
  const session = ctx.session;
